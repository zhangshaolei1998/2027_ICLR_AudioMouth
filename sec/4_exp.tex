...
\input{figures/ablation_combined_figure}

\noindent\textbf{Ablation of Linguistic and Articulatory Conditioning.}
To examine the role of explicit linguistic inputs, we compare three SFT variants using the same segmented audio: Audio Only, Audio + Text + Phonemes, and Audio + Text + Phonemes + Guidance (Figure~\ref{fig:ablations}(a)). Text denotes the transcript, and Guidance denotes articulation instructions.
...
To assess the effect of coefficient-value imbalance, we compare the weighted SFT objective with SFT without Reweighting, which assigns unit weight to coefficient-value tokens while retaining the same conditioning and post-processing (Figure~\ref{fig:ablations}(b)).
...
To evaluate whether motion-level feedback complements token-level learning, we compare the SFT generator with the full GRPO-refined generator under the same post-processing setting (Figure~\ref{fig:ablations}(c)).
...
To assess the contribution of geometric feedback within refinement, we compare the full reward with GRPO without SLMD, which omits the simplified 3D lip-geometry term (Figure~\ref{fig:ablations}(c)).
